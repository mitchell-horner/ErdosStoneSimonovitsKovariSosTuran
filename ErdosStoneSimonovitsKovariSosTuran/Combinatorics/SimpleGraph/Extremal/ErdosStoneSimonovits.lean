...
lemma degree_between_verts_lt_of_mem_sdiff
  ...
  rw [← card_neighborFinset_eq_degree,
    ...
    K.card_filter_verts _, sum_eq_sum_diff_singleton_add hp]
  apply add_lt_add_of_le_of_lt
  · conv_rhs =>
      rw [K.card_verts, ← Nat.sub_one_mul, ← K.card_parts.resolve_right ht'_pos.ne',
        ← card_singleton p, ← card_sdiff (singleton_subset_iff.mpr hp),
...
lemma exists_induce_minDegree_ge_and_card_edgeFinset_ge {V : Type*} [Fintype V]
  ...
  · replace hδ : (G.induce G.support).minDegree < c * (card G.support) := by
      ...
      exact hδ.trans_eq' (by convert rfl <;> exact G.support.coe_toFinset)
...
lemma exists_induce_minDegree_ge_and_card_sq_ge {V : Type*} [Fintype V]
  ...
  classical obtain ⟨s, -, hδ, hs⟩ := exists_induce_minDegree_ge_and_card_edgeFinset_ge hc_nonneg G
  refine ⟨s, hδ, ?_⟩
...
theorem eventually_isContained_of_card_edgeFinset_of_colorable
  ...
  have ⟨N, ih⟩ := eventually_atTop.mp <|
    eventually_completeEquipartiteGraph_isContained_of_card_edgeFinset hε_pos r
      (univ.sup fun c ↦ Fintype.card (C.colorClass c))
...
/-- If `G` has at least `(1 - 1 / r + o(1)) * n.choose 2` many edges, then `G`
