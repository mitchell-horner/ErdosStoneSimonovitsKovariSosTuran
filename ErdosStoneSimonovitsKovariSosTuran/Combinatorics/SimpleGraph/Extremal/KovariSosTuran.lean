  ...
  · have h_isBipartiteWith : G.IsBipartiteWith
    ...
    · rw [ge_iff_le, ← le_div_iff₀ (mod_cast card_pos)] at h_avg
      ...
      have h : (card W * (#G.edgeFinset / card W - card α + 1) ^ card α / (card α).factorial : ℝ) ≤
        ...
        -- counting `v`
        · exact (card_filter_le h_free).trans <| mul_le_mul_of_nonneg_right
            (Nat.choose_le_pow_div (card α) (card V))
      ...
      calc (#G.edgeFinset : ℝ)
        ...
        _ ≤ card W * ((card β - 1) ^ (card α : ℝ)⁻¹ * card V / card W ^ (card α : ℝ)⁻¹)
              + (card α - 1) * card W :=
            add_le_add_right (mul_le_mul_of_nonneg_left h_root (Nat.cast_nonneg _)) _
